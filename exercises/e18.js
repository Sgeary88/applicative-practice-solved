...
export function getGreatestDiscoveryYear(data) {
  const years = data.asteroids.reduce((acc, asteroid) => {
	...
	const maxYear = Object.entries(years)
	.reduce((max, [year, count]) => (count > max.count ? {year, count} : max), {year: null, count: 0}).year;
